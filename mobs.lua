...
local function taint(pos, spread)
	minetest.find_items_in_area(
		{x = pos.x+spread, y = pos.y+spread, z = pos.z-spread},
		{x = pos.x-spread, y = pos.y-spread, z = pos.z-spread},
		tainted_soils,
	)
end

local function taintgen(chance, spread)
	if not spread then
		spread = 1
	end
...
local function diegen(mob)
	return function(pos)
		mobs:spawnentity(mob) -- FIXME
	end
end
...
mobs:register_mob("zombies:zombie", {
	...
	do_custom = taintgen(20),
	on_die = diegen("zombie:1arm")
})
...
mobs:register_mob('zombies:normal', {
	...
	do_custom = taintgen(40),
	on_die = diegen("zombie:crawler")
})
